...
export function CSMRenewalView() {
  ...
  // Per the hierarchy redesign: accounts are now primary. CSM grouping is
  // dropped at the top level — each opp's CSM is still shown on its
  // OpportunityCard. Opps stay sorted by close date inside each account.
    ...
    <div className="prs-view">
      {isAdmin && (
        <div className="ui-banner ui-banner--info" style={{ marginBottom: "var(--space-4)" }}>
          ...
          <span>Viewing all {uniqueAccounts} accounts</span>
        </div>
      )}
